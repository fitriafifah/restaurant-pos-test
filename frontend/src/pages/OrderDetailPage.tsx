...
import {
  ...
  MenuItem as MuiMenuItem, Select, CircularProgress
} from '@mui/material';
...
type Order = { id:number; table_number:number; status:'open'|'closed'; items:Item[]; total:number; };
...
export default function OrderDetailPage(){
  ...
  return (
    <Box p={2}>
      <Typography variant="h5" mb={2}>Order Meja #{order.table_number}</Typography>
      ...
      <Typography mt={2} variant="h6">Total: Rp {order.total.toLocaleString()}</Typography>
      ...
      {order.status==='open' && user?.role==='kasir' && (
        <Button color="error" variant="contained" sx={{mt:2, mr:2}}
          onClick={()=>closeOrder.mutate()} disabled={closeOrder.isPending}>
          Tutup Order
        </Button>
      )}

      {/* Receipt (kasir, closed) */}
